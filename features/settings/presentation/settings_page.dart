...
class SettingsPage extends StatelessWidget {
    ...
    return Scaffold(
      ...
      body: ListView(
        children: [
          ...
          _buildTile('Data Management', Icons.database_outlined, () {}),
          _buildDivider(),
          _buildSection('About'),
  ...
  Widget _buildSection(String title) {
    return Padding(padding: const EdgeInsets.fromLTRB(20, 20, 20, 8), child: Text(title, style: AppTextStyles.bodySmall.copyWith(color: AppColors.onSurfaceVariant, fontWeight: FontWeight.w600), textAlign: TextAlign.left));
  }

  Widget _buildTile(String title, IconData icon, VoidCallback onTap) {
    return InkWell(onTap: onTap, child: Container(color: AppColors.surface, padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 16), child: Row(children: [Icon(icon, size: 24, color: AppColors.onSurfaceVariant), const SizedBox(width: 16), Expanded(child: Text(title, style: AppTextStyles.bodyMedium)), const Icon(Icons.chevron_right, size: 24, color: AppColors.onSurfaceVariant)])));
  }

  Widget _buildDivider() {
    return Container(margin: const EdgeInsets.only(left: 56), height: 1, color: AppColors.surfaceContainerHigh);
  }
}
