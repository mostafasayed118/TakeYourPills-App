...
class MedicationListPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => MedicationListCubit(context.read())..loadMedications(),
          ...
          builder: (context, state) {
              ...
              return Center(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Error: ${state.message}', style: AppTextStyles.bodyMedium),
                    const SizedBox(height: 16),
                    AppButton(
                      text: 'Retry',
                      onPressed: () => context.read<MedicationListCubit>().loadMedications(),
                      isPrimary: true,
                    ),
                  ],
                ),
            ...
            if (state is MedicationListEmpty) {
              return EmptyStateWidget(
                ...
                icon: const Icon(Icons.medication, size: 80, color: AppColors.surfaceContainerHigh),
          ...
          child: Row(
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: isPaused ? AppColors.surfaceContainerLow : AppColors.secondaryContainer,
                  borderRadius: BorderRadius.circular(12),
                ),
                child: Icon(Icons.medication, size: 28, color: isPaused ? AppColors.onSurfaceVariant : AppColors.onSecondaryContainer),
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('${med.dosageAmount} ${med.dosageUnit}', style: AppTextStyles.bodySmall.copyWith(color: AppColors.onSurfaceVariant)),
                    const SizedBox(height: 2),
                    Text('${med.scheduleTimes} • ${med.frequencyType}', style: AppTextStyles.bodySmall.copyWith(color: AppColors.onSurfaceVariant)),
                    if (isPaused)
                      Padding(
                        ...
                        child: Container(
                          ...
                          child: Text('PAUSED', style: AppTextStyles.bodySmall.copyWith(fontSize: 9, color: AppColors.onSurfaceVariant)),
                        ),
                      ),
                    if (med.pillsRemaining != null)
                      Padding(
                        padding: const EdgeInsets.only(top: 4),
                        child: Text('${med.pillsRemaining} pills remaining', style: AppTextStyles.bodySmall.copyWith(color: AppColors.primary)),
                      ),
                  ],
                ),
              ),
              PopupMenuButton<String>(
                ...
                itemBuilder: (context) => [
                  ...
                  PopupMenuItem(value: med.isPaused ? 'resume' : 'pause', child: Text(med.isPaused ? 'Resume' : 'Pause')),
