...
class AppDatabase extends _$AppDatabase {
  ...
  Future<int> updateMedicationRow(MedicationData medication) => (update(medications)
          ..where((t) => t.id.equals(medication.id)))
        .write(medication);

  Future<int> deleteMedication(int id) => (delete(medications)..where((t) => t.id.equals(id))).go();

  Future<MedicationData?> getMedicationById(int id) => (select(medications)..where((t) => t.id.equals(id)))
        .getSingleOrNull();

  Future<List<MedicationData>> getAllMedications() => (select(medications)
          ..orderBy([(t) => OrderingTerm.desc(t.createdAt)]))
        .get();

  Future<List<MedicationData>> getActiveMedications() => (select(medications)..where((t) => t.isPaused.equals(false))).get();

  Stream<List<MedicationData>> watchAllMedications() => (select(medications)
          ..orderBy([(t) => OrderingTerm.desc(t.createdAt)]))
        .watch();
  ...
  Future<int> updateScheduleRow(SchedulesCompanion schedule) => (update(schedules)
          ..where((t) => t.id.equals(schedule.id.value)))
        .write(schedule);

  Future<int> deleteSchedule(int id) => (delete(schedules)..where((t) => t.id.equals(id))).go();

  Future<int> deleteSchedulesForMedication(int medicationId) => (delete(schedules)
          ..where((t) => t.medicationId.equals(medicationId)))
        .go();

  Future<List<ScheduleData>> getSchedulesForMedication(int medicationId) => (select(schedules)
          ..where((t) => t.medicationId.equals(medicationId)))
        .get();
  ...
  Future<List<DoseLogData>> getDoseLogsForMedication(int medicationId) => (select(doseLogs)
          ..where((t) => t.medicationId.equals(medicationId)))
        .get();
